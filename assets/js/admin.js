...
function renderBlocks(){
  ...
  el.innerHTML = pBlocks.length ? pBlocks.map((b,i)=>{
    const head = `<div class="blk-head"><b>${b.type==='p'?'문단':b.type==='img'?'이미지':'영상'}</b><div class="acts"><button onclick="moveBlock(${i},-1)" ${i===0?'disabled':''}>↑</button><button onclick="moveBlock(${i},1)" ${i===pBlocks.length-1?'disabled':''}>↓</button><button onclick="delBlock(${i})">삭제</button></div></div>`;
    if(b.type==='p'){
      const tx = b.text||{};
